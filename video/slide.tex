...
\begin{frame}{Une première méthode: ISTA}
    On veut minimizer: $F(x) = f(x) + g(x)$ avec $f=\frac{1}{2}\lVert y - H \rVert ^2$ et $g=\lambda \Phi$
    
    On prend $L$ la constante de Lipschitz de $f$.

    On pose $$ p_L(y) = \arg \max_x \Bigg\{ g(x) + \frac{L}{2} \Big\lVert x - \Big(y - \frac{1}{L} \nabla f(y)\Big) \Big\rVert ^2 \Bigg\}$$
\end{frame}
